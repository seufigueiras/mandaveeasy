...
app.post(['/api/whatsapp-webhook', '/api/webhook/messages'], async (req, res) => {
  try {
    console.log('\n📱 ====================================');
...
// 🟢 CORREÇÃO CRÍTICA PARA SERVIR SPA (Deve ser a última rota)
// Qualquer rota que não tenha sido tratada deve retornar o index.html do frontend.
// Usamos .use() como fallback, o que é mais robusto que .get('*').
app.use((req, res) => {
    // Verifica se a rota é uma API que falhou (404), senão, serve o index.html
    if (!req.path.startsWith('/api/')) {
        res.sendFile(path.join(__dirname, 'dist', 'index.html'));
    } else {
        res.status(404).json({ error: 'Not Found', message: `Rota de API ${req.path} não encontrada.` });
    }
});
